...
export default function UserDashboard() {
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      doc.text(selectedPDF.name, margin, margin);

      // Add content
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');

      const content = isEditing ? editingContent : selectedPDF.content;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(
          `Exported from PDFRooms | Page ${i} of ${totalPages} | ${new Date().toLocaleDateString()}`,
          margin,
          pageHeight - 10
        );
      }
      ...
      // Save the PDF
      const fileName = selectedPDF.name.replace('.pdf', '') + '_exported.pdf';
      doc.save(fileName);
      ...
      if (roomIndex !== -1) {
        logActivity(allRooms[roomIndex].id, {
          ...
          pdfId: selectedPDF.id,
          pdfName: selectedPDF.name,
          description: `Exported PDF "${selectedPDF.name}" as "${fileName}"`
        });
      }
